...
int main() {
    ...
    FILE* key_file = fopen("EXAM_2024/RSAPrivateKey.pem", "r");
    if (!key_file) {
    ...
    size_t accounts_file_size = read_file("EXAM_2024/Accounts.txt", &accounts_file_data);
    if (accounts_file_size == 0) return -1;

    // Compute SHA-256 for each line in Accounts.txt
    FILE* signatures_file = fopen("EXAM_2024/AllSigns.sig", "wb");
    if (!signatures_file) {
    ...
    size_t all_signs_file_size = read_file("EXAM_2024/AllSigns.sig", &all_signs_file_data);

    FILE* sign1_file = fopen("EXAM_2024/Sign1.sig", "wb");
    FILE* sign2_file = fopen("EXAM_2024/Sign2.sig", "wb");
    ...
    FILE* aes1_file = fopen("EXAM_2024/aes1.enc", "wb");
    if (!aes1_file) {
    ...
    FILE* aes2_file = fopen("EXAM_2024/aes2.enc", "wb");
    if (!aes2_file) {
