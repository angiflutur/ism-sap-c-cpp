...
int main()
{
	//1st req
	SHA256_CTX ctx;
	FILE* f = fopen("EXAM_IAN/name.txt", "r");
	fseek(f, 0, SEEK_END);
	size_t len = ftell(f);
	fseek(f, 0, SEEK_SET);
	...
	f = fopen("EXAM_IAN/enc_name.aes", "wb");
	...
	f = fopen("EXAM_IAN/digital.sign", "wb");
